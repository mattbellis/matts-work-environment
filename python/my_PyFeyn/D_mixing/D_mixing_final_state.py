...
def D_mix_final_state(outfilename, stage=0):
    #from pyfeyn.user import *
    ...
    q.addLabel(r"\Pqs",pos=0.33,displace=0.05)
    q.addLabel(r"\Pqs",pos=0.66,displace=0.05)
    q.addLabel(r"\PDz",pos=-0.10,displace=-0.50,size=pyx.text.size.Large)
    q.addLabel(r"\textcolor{COL}{\PKm}",pos=0.55,displace=0.50,size=pyx.text.size.Large)

    qbar = Fermion(qbar_in, qbar_out)
    qbar.addArrow(1.02)
    qbar.addLabel(r"\Paqu",pos=-0.05,displace=0.00)
    qbar.addLabel(r"\Paqu",pos=1.05,displace= 0.01)

    ############# W ##########################
    Wpts = [Point(-2.0, q_y_pos), Point(-1.0, q_y_pos+0.7)]
    ex_0 = Higgs(Wpts[0],Wpts[1]).addLabel(r"\PWp", pos=0.50, displace=-0.30,size=pyx.text.size.footnotesize)
    ex_0.addArrow(0.50)

    Wpts_1 = [Point(2.0, q_y_pos), Point(1.0, q_y_pos+0.7)]
    ex_1 = Higgs(Wpts_1[1],Wpts_1[0]).addLabel(r"\PWp", pos=0.50, displace=-0.30,size=pyx.text.size.footnotesize)
    ex_1.addArrow(0.50)

    ############# pi ##########################
    pi_pts = decay_line(Wpts,20,0.8)
    q_0 = Fermion(pi_pts[0],pi_pts[1])
    q_0.addLabel(r"\Pqu",pos=1.05,displace=-0.01,size=pyx.text.size.tiny)

    pi_pts = decay_line(Wpts,-20.0,0.8)
    q_1 = Fermion(pi_pts[0],pi_pts[1])
    q_1.addLabel(r"\Paqs",pos=1.05,displace=0.01,size=pyx.text.size.tiny)
    #q_1.addLabel(r"\PKp",pos=1.45,displace=-0.10,size=pyx.text.size.Large)
    q_1.addLabel(r"\textcolor{COL}{\PKp}",displace=-0.10,pos=1.45,size=pyx.text.size.Large)
