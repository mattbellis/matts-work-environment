...
#  from pyfeyn.user import *
  ...
  n_str = []
  n_str.append(['gpp',['qu','aqd']])
  n_str.append(['gpm',['qd','aqu']])
  n_str.append(['Jgy',['qc','aqc']])
  n_str.append(['gU',['aqb','qb']])



  q_rad = []
  q_rad.append([0.3,0.3])
  q_rad.append([0.3,0.3])
  q_rad.append([0.5,0.5])
  q_rad.append([0.75,0.75])

  q_dis = []
  q_dis.append([0.35,0.35])
  q_dis.append([0.35,0.35])
  q_dis.append([0.55,0.55])
  q_dis.append([0.75,0.75])
  ...
  for i in range(0,option):
      ...
      n_y.append(y0+n_dis[i]*sin(a))
      n_pt = Point(n_x[i],n_y[i])
      mesons.append(Circle(center=n_pt, radius=n_r[i], fill=[TAN]).addLabel(r"\P"+n_str[i][0],displace=n_r[i]+0.5,angle=45,size=pyx.text.size.large))
      ...
      for j,k in enumerate(q_color):
          ...
          if option>=0:
              ...
              if j==1:
                  ...
                  aq.append(Circle(center=q_pt, radius=q_rad[i][j]/1.5, fill=[BLACK]).addLabel(r"\textcolor{COL}{\P"+n_str[i][1][j]+"}",displace=0.01,
