...
def epem_B_lambda0(outfilename, stage=0):
    ...
    if stage>0:
        Bbar = Fermion(Bbar_in, Bbar_out).addLabel(r"\PBp", pos=1.00, displace=0.18).addArrow(1.05)
    ...
    if stage>0:
        B = Fermion(B_in, B_out).addLabel(r"\PBm", pos=1.05, displace=0.18).addArrow(1.05).setStyles([CARNATIONPINK,THICK3])
    ...
    if stage>2:
        lam = Fermion(fin, fout).addArrow(1.05).addLabel(r"\PgL",pos=0.9,displace=0.20).setStyles([CARNATIONPINK,THICK3])
