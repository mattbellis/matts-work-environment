...
def epem_B_lambdac(outfilename, stage=0):
    ...
    if stage>0:
        Bbar = Fermion(Bbar_in, Bbar_out).addLabel(r"\APBzero", pos=1.00, displace=0.18).addArrow(1.05)
    ...
    if stage>0:
        B = Fermion(B_in, B_out).addLabel(r"\PBzero", pos=1.05, displace=0.18).addArrow(1.05).setStyles([CARNATIONPINK,THICK3])
