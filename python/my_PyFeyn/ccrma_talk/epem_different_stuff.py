...
def epem_B_lambdac(outfilename, stage=0):
    ...
    if stage>0:
        Bbar = Fermion(Bbar_in, Bbar_out).addLabel(r"$X$", pos=1.00, displace=0.18).addArrow(1.05).setStyles([CARNATIONPINK,THICK3])
    ...
    if stage>0:
        B = Fermion(B_in, B_out).addLabel(r"$\bar{X}$", pos=1.05, displace=0.18).addArrow(1.05).setStyles([CARNATIONPINK,THICK3])
    ...
    fout = Point(xout, yout)
    if stage>2:
        muon = Fermion(fin, fout).addArrow(1.05).addLabel(r"\Plm",pos=0.9,displace=0.20).setStyles([AQUAMARINE,THICK2])
    ...
    if stage>2:
        lam = Fermion(fin, fout).addArrow(1.05).addLabel(r"\PcgLp",pos=0.9,displace=0.20).setStyles([CARNATIONPINK,THICK2])
    ...
    for i,a in enumerate(lam_decay_angles):
        ...
        fout = Point(xout, yout)
        if stage>3:
            lam_decay.append(Fermion(fin, fout).addArrow(1.05).addLabel(r"\P"+lam_decay_strings[i],pos=1.2,displace=0.00).setStyles([AQUAMARINE,THICK2]))
    '''
