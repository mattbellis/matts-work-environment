...
__global__ void CalcSep(float* raA, float* sin_decA, float* cos_decA, int ngals, int nthreads, int* hist_array, float hist_lo, float hist_hi, int hist_nbins)
{
	...
	// need shared array that in nbins * nthreads
	//
	
	if (hist_nbins>2)	
	{
		for (int j = 0; j<hist_nbins; ++j){
			s_data[threadIdx.x*hist_nbins+j] = 0;
		}
	}
...
//    int hist_array_bin_block = ix*hist_nbins;
	int hist_array_bin_block = threadIdx.x*hist_nbins;
	...
	float ra_ij;
		...
		/*
	for(int k = 0; k< ngals; ++k){
		sin_dec[k]=sin_decA[k];
		cos_dec[k]=cos_decA[k];
		ra[k]=raA[k];
	}
	*/
	...
	for(int ij=ix+1;ij<ngals;ij++)
    {
        ...
        if (hist_nbins>2 )
        {
			if(index<nthreads) s_data[hist_array_bin]++;	
			//hist_array[hist_array_bin]++;
		}
	...
	// Now load s_data into 
		//for(int kbin =0; kbin<10; kbin++) {
	//s_data[threadIdx.x]=99;
	int hist_hostblock = blockIdx.x*hist_nbins;
	
	if(hist_nbins >2)// && threadIdx.x<hist_nbins) 
	{
		int thisbin = threadIdx.x;
		for(int thisbin=threadIdx.x; thisbin < hist_nbins; thisbin+=blockDim.x) 
		{				
			hist_hostblock	= thisbin*hist_nbins;
			for(int k =1; k < blockDim.x; k++)
			{
				//s_data[(hist_nbins*k) +threadIdx.x]=99;
				s_data[thisbin]+=s_data[(hist_nbins*k) + thisbin];
				//hist_array[hist_hostblock+threadIdx.x]+=s_data[(hist_nbins*k) +threadIdx.x];
			}
			hist_array[hist_hostblock+thisbin] = s_data[thisbin];
			//hist_array[hist_hostblock+threadIdx.x] =	 blockIdx.x;
		}
	}
}
///////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char **argv)
{
	...
	int nblocks = 256;//ceil(nthreads/(threadsPerBlock*1.0));
	int threadsPerBlock = ceil(nthreads/(nblocks*1.0));
	
	//int threadsPerBlock = 64;
	
	if(threadsPerBlock<nbins_with_overflow)
	{
		printf("Too many bins for this blocksize\n");
	}
	
	
	
	
	//int threadsPerBlock = ceil(nthreads/(nblocks*1.0));//nbins_with_overflow;

	//if (! nthreads%nblocks) threadsPerBlock++;
	...
	printf("shared memory allocated: %d\n",threadsPerBlock*nbins_with_overflow*sizeof(int));

    // Calculate our thread/grid/block sizes.
	//int threadsPerBlock = ;
	    // IS THIS CALCULATION BEING DONE PROPERLY? OPTIMALLY?????
    int blocksPerGrid = nblocks ;//(nthreads/threadsPerBlock)+1; //(nthreads + threadsPerBlock -1) / threadsPerBlock; //???????
    ...
    CalcSep<<<blocksPerGrid, threadsPerBlock, threadsPerBlock*nbins_with_overflow*sizeof(int) >>>(d_raA, d_sin_decA, d_cos_decA,
															ngals, nthreads, d_hist_array, hist_lo, hist_hi, nbins_with_overflow);
