# Translate http://root.cern.ch/root/html/tutorials/roofit/rf305_condcorrprod.C.html
# to pyroot
...
xframe = x.frame()
argData = ROOT.RooLinkedList()
argData.Add(ROOT.RooFit.MarkerStyle(21))
data.plotOn(xframe,argData)
model.plotOn(xframe) 

# Plot x distribution of data and projection of model on y = Int(dx) model(x,y)
yframe = y.frame()
data.plotOn(yframe,argData)
model.plotOn(yframe) 
