...
n12frac = RooRealVar("n12frac","Ratio of n12",0.40, 0.0,1.0)

gauss1 = RooGaussian("gauss1","Unconstrained #DeltaT Gaussian 1",time, mean1, sigma1)
gauss2 = RooGaussian("gauss2","Unconstrained #DeltaT Gaussian 2",time, mean2, sigma2)
gauss3 = RooGaussian("gauss3","Unconstrained #DeltaT Gaussian 3",time, mean3, sigma3)
...
#total = RooAddPdf("total","total",RooArgList(gauss1,gauss2,gauss3),RooArgList(n1,n2,n3))
tot12 = RooAddPdf("tot12","tot12",RooArgList(gauss1,gauss2),RooArgList(n12frac))
total = RooAddPdf("total","total",RooArgList(tot12,gauss3),RooArgList(n1,n2))
...
n1val = n1.getVal(); print "n1val: %f" % (n1val); n1.setVal(n1val); n1.setConstant(kTRUE);
n2val = n2.getVal(); print "n2val: %f" % (n2val); n2.setVal(n2val); n2.setConstant(kTRUE);
n3val = n3.getVal(); print "n3val: %f" % (n3val); n3.setVal(n3val); n3.setConstant(kTRUE);
n12fracval = n12frac.getVal(); print "n12frac: %f" % (n12fracval); n12frac.setVal(n12fracval); n12frac.setConstant(kTRUE);
Dataset = total.generate(timeVars)

print "Fitting to toy data..."
n1.setConstant(kFALSE);
n2.setConstant(kFALSE);
n3.setConstant(kFALSE);
n12frac.setConstant(kFALSE);

fit_result = total.fitTo(Dataset,RooFit.Extended(kTRUE),RooFit.Save(kTRUE),RooFit.Strategy(2))
