...
def build_xyz(var_ranges=[[5.2,5.3],[-0.2,0.2],[0.75,1.0]]):
    ...
    x = RooRealVar("x","m_{ES}",xlo, xhi, "GeV/c^{2}")
    y = RooRealVar("y","#Delta E",ylo,yhi, "GeV")
    z = RooRealVar("z","NN output",zlo,zhi)
...
def tot_PDF(x,y,z, dataset, dim = 2, use_double_CB=False, workspace=None):
    ...
    conv_factor_calc = RooRealVar("conv_factor_calc","Conversion factor (calculated)",13.272) # Conversion factor, calculated
    conv_factor_fit  = RooRealVar("conv_factor_fit", "Conversion factor (fit)",13.272) # Conversion factor, fit
    conv_factor_err  = RooRealVar("conv_factor_err", "Error on conversion factor",0.32) # Conversion factor, fit

    branching_fraction = RooRealVar("branching_fraction","Branching fraction",12.2)

    nbkg = RooRealVar("nbkg","# bkg events,",150)
    nsig = RooFormulaVar("nsig","conv_factor_fit*branching_fraction",RooArgList(conv_factor_fit,branching_fraction))
    #nsig = RooRealVar ("nsig","# sig events",150)

    # Gaussian constraint
    #gc = RooFormulaVar("gc","exp(-(conv_factor_fit-conv_factor_calc)*(conv_factor_fit-conv_factor_calc)/(2.0*conv_factor_err*conv_factor_err))", \
    #RooArgList(conv_factor_fit,conv_factor_calc,conv_factor_fit,conv_factor_calc,conv_factor_err,conv_factor_err))
    log_gc = RooFormulaVar("log_gc","(conv_factor_calc-conv_factor_fit)*(conv_factor_calc-conv_factor_fit)/(2.0*conv_factor_err*conv_factor_err)", \
            RooArgList(conv_factor_calc,conv_factor_fit,conv_factor_calc,conv_factor_fit,conv_factor_err,conv_factor_err))


    #sig_temp = RooGenericPdf("sig_temp","gc*sig_pdf", RooArgList(gc,sig_pdf))
    #bkg_temp = RooGenericPdf("bkg_temp","gc*bkg_pdf", RooArgList(gc,bkg_pdf))

    #total = RooAddPdf("total","sig_temp + bkg_temp", RooArgList(sig_temp, bkg_temp), RooArgList(nsig, nbkg))
    #sub_total = RooAddPdf("sub_total","sig_pdf + bkg_pdf", RooArgList(sig_pdf, bkg_pdf), RooArgList(nsig, nbkg))
    #sub_total = RooAddPdf("sub_total","sig_pdf + bkg_pdf", RooArgList(sig_pdf, bkg_pdf), RooArgList(nsig, nbkg))

    #total = RooGenericPdf("total","gc*sub_total", RooArgList(gc,sub_total))
