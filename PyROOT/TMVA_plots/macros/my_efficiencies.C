...
void plot_efficiencies( TFile* file, TString outfilename, Int_t type = 2, TDirectory* BinDir)
{
   ...
   TString fname = "plots/" + hNameRef + "_" + outfilename.Data();
   if (TString(BinDir->GetName()).Contains("multicut")){
...
void my_efficiencies( TString fin = "TMVA.root", TString outfilename="default", Int_t type = 2, Bool_t useTMVAStyle = kTRUE )
{
   ...
   plot_efficiencies( file, outfilename, type, gDirectory );
