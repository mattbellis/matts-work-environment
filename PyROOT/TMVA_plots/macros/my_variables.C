...
void my_variables( TString fin = "TMVA.root", TString outfilename="default", TString dirName = "InputVariables_Id", TString title = "TMVA Input Variables",
                Bool_t isRegression = kFALSE, Bool_t useTMVAStyle = kTRUE )
{
   ...
   while ((key = (TKey*)next())) {
      ...
      // save canvas to file
      if (countPad%noPadPerCanv==0) {
         cerr << outfilename.Data() << endl;
         cerr << outfname.Data() << endl;
         TString fname = Form( "plots/%s_c%i", outfilename.Data(), countCanvas );
