...
//void my_mvas( TString fin = "TMVA.root", TString outfilename="default", HistType htype = MVAType, Bool_t useTMVAStyle = kTRUE )
void my_mvas( TString fin = "TMVA.root", TString outfilename="default", HistType htype = 3, Bool_t useTMVAStyle = kTRUE )
{
   ...
   while ((key = (TKey*)next())) {
      ...
      while ((titkey = (TKey*)keyIt())) {
         ...
         //TMVAStyle->SetFrameFillColor(0);
         //TMVAStyle->SetFrameFillStyle(1001);
         ...
         char name[256];
         sprintf(name,"%s",methodName.Data());
         TString test(name);
         ...
         if (Save_Images) {
            ...
            else if (htype == CompareType) TMVAGlob::imgconv( c, Form("plots/overtrain_%s_%s", methodName.Data(), outfilename.Data()) ); 
