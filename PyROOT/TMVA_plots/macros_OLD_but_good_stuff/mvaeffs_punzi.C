...
class StatDialogMVAEffs_punzi {  

   RQ_OBJECT("StatDialogMVAEffs_punzi")
      
public:

   StatDialogMVAEffs_punzi(const TGWindow* p, Float_t ns, Float_t nb);
   virtual ~StatDialogMVAEffs_punzi();
...
void StatDialogMVAEffs_punzi::SetNSignal() 
{
   fNSignal = fSigInput->GetNumber();
}

void StatDialogMVAEffs_punzi::SetNBackground() 
{
   fNBackground = fBkgInput->GetNumber();
}

void StatDialogMVAEffs_punzi::Setasig() 
{
   fasig = fasigInput->GetNumber();
}

TString StatDialogMVAEffs_punzi::GetFormula() 
{
   TString f = fFormula;
   f.ReplaceAll("S","x");
   f.ReplaceAll("B","y");
   char sasig[256];
   sprintf(sasig,"%f",fasig);
   f.ReplaceAll("asig",sasig);
   return f;
}

TString StatDialogMVAEffs_punzi::GetLatexFormula() 
{
   TString f = fFormula;
   f.ReplaceAll("(","{");
   f.ReplaceAll(")","}");
...
void StatDialogMVAEffs_punzi::Redraw() 
{
   UpdateSignificanceHists();
   UpdateCanvases();
}

void StatDialogMVAEffs_punzi::Close() 
{
   delete this;
}

StatDialogMVAEffs_punzi::~StatDialogMVAEffs_punzi() 
{
   if (fInfoList) { 
...
StatDialogMVAEffs_punzi::StatDialogMVAEffs_punzi(const TGWindow* p, Float_t ns, Float_t nb) :
   fNSignal(ns),
   fNBackground(nb),
   fFormula(""),
   fInfoList(0),
   ...
   maxLenTitle(0)
{
   ...
   TGLabel *asigLab = new TGLabel(fMain, "a significance");
   fMain->AddFrame(asigLab, new TGLayoutHints(kLHintsLeft | kLHintsTop,5,5,5,5));

   fasigInput = new TGNumberEntry(fMain, (Double_t) fNBackground,5,-1,(TGNumberFormat::EStyle) 5);
   fasigInput->SetLimits(TGNumberFormat::kNELLimitMin,0,1);
   fMain->AddFrame(fasigInput, new TGLayoutHints(kLHintsLeft | kLHintsTop,5,5,5,5));
   fasigInput->Resize(100,24);
   ...
   fSigInput->Connect("ValueSet(Long_t)","StatDialogMVAEffs_punzi",this, "SetNSignal()");
   fBkgInput->Connect("ValueSet(Long_t)","StatDialogMVAEffs_punzi",this, "SetNBackground()");
   fasigInput->Connect("ValueSet(Long_t)","StatDialogMVAEffs_punzi",this, "Setasig()");
   ...
   fDrawButton->Connect("Clicked()","TGNumberEntry",fasigInput, "ValueSet(Long_t)");
   fDrawButton->Connect("Clicked()", "StatDialogMVAEffs_punzi", this, "Redraw()");   

   fCloseButton->Connect("Clicked()", "StatDialogMVAEffs_punzi", this, "Close()");
}

void StatDialogMVAEffs_punzi::UpdateCanvases() 
{
...
void StatDialogMVAEffs_punzi::UpdateSignificanceHists() 
{
   ...
   //TString str = Form( "%*s   (  #signal, #backgr.)  Optimal-cut  S/sqrt(S+B)      NSig      NBkg   EffSig   EffBkg", maxLenTitle, cname.Data() );
   TString str = Form( "%*s   (  #signal, #backgr.)  Optimal-cut  S/(sqrt(B)+a/2)     NSig      NBkg   EffSig   EffBkg", maxLenTitle, cname.Data() );
   ...
   while ((info = (MethodInfo*)next())) {
      for (Int_t i=1; i<=info->origSigE->GetNbinsX(); i++) {
         ...
         info->sSig->SetBinContent( i, f.Eval(S,B, asig) );
...
void StatDialogMVAEffs_punzi::ReadHistograms(TFile* file) 
{
   if (fInfoList) { 
      TIter next(fInfoList);
      MethodInfo *info(0);
...
void StatDialogMVAEffs_punzi::DrawHistograms() 
{
   // counter variables
   Int_t countCanvas = 0;
   ...
   while ( (info = (MethodInfo*)next()) ) {
      ...
      legend2->AddEntry(info->sSig,"S / (#sqrt{B}+a/2)","L");
      legend2->Draw("same");
      ...
      tl.DrawLatex( 0.15, 0.19, "events the maximum S / (#sqrt{B}+a/2) is");
      ...
      if (Save_Images) {
         TMVAGlob::imgconv( c, Form("plots/MVAEffs_punzi_%s", info->methodTitle.Data()) ); 
      }
      countCanvas++;
   }
}

void StatDialogMVAEffs_punzi::PrintResults( const MethodInfo* info )
{
   Int_t maxbin = info->sSig->GetMaximumBin();
   if (info->line1 !=0 )
...
//void mvaeffs_punzi( TString fin = "TMVA.root", Bool_t useTMVAStyle = kTRUE, TString formula="S/sqrt(B)" )
void mvaeffs_punzi( TString fin = "TMVA.root", Bool_t useTMVAStyle = kTRUE, TString formula="S/(sqrt(B) + asig/2.0)" )
{
   TMVAGlob::Initialize( useTMVAStyle );

   StatDialogMVAEffs_punzi* gGui = new StatDialogMVAEffs_punzi(gClient->GetRoot(), 1, 1);
