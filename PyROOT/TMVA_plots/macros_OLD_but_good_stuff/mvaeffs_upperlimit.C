...
class StatDialogMVAEffs_upperlimit {  

   RQ_OBJECT("StatDialogMVAEffs_upperlimit")
      
public:

   StatDialogMVAEffs_upperlimit(const TGWindow* p, Float_t ns, Float_t nb);
   virtual ~StatDialogMVAEffs_upperlimit();
...
void StatDialogMVAEffs_upperlimit::SetNSignal() 
{
   fNSignal = fSigInput->GetNumber();
}

void StatDialogMVAEffs_upperlimit::SetNBackground() 
{
   fNBackground = fBkgInput->GetNumber();
}

TString StatDialogMVAEffs_upperlimit::GetFormula() 
{
   TString f = fFormula;
   f.ReplaceAll("S","x");
   f.ReplaceAll("B","y");
...
TString StatDialogMVAEffs_upperlimit::GetLatexFormula() 
{
   TString f = fFormula;
   f.ReplaceAll("(","{");
   f.ReplaceAll(")","}");
...
void StatDialogMVAEffs_upperlimit::Redraw() 
{
   UpdateSignificanceHists();
   UpdateCanvases();
}

void StatDialogMVAEffs_upperlimit::Close() 
{
   delete this;
}

StatDialogMVAEffs_upperlimit::~StatDialogMVAEffs_upperlimit() 
{
   if (fInfoList) { 
...
StatDialogMVAEffs_upperlimit::StatDialogMVAEffs_upperlimit(const TGWindow* p, Float_t ns, Float_t nb) :
   fNSignal(ns),
   fNBackground(nb),
   fFormula(""),
   fInfoList(0),
   ...
   maxLenTitle(0)
{
   ...
   fSigInput->Connect("ValueSet(Long_t)","StatDialogMVAEffs_upperlimit",this, "SetNSignal()");
   fBkgInput->Connect("ValueSet(Long_t)","StatDialogMVAEffs_upperlimit",this, "SetNBackground()");
   ...
   fDrawButton->Connect("Clicked()", "StatDialogMVAEffs_upperlimit", this, "Redraw()");   

   fCloseButton->Connect("Clicked()", "StatDialogMVAEffs_upperlimit", this, "Close()");
}

void StatDialogMVAEffs_upperlimit::UpdateCanvases() 
{
...
void StatDialogMVAEffs_upperlimit::UpdateSignificanceHists() 
{
   ...
   TString str = Form( "%*s   (  #signal, #backgr.)  Optimal-cut  S/sqrt(B)      NSig      NBkg   EffSig   EffBkg", maxLenTitle, cname.Data() );
...
void StatDialogMVAEffs_upperlimit::ReadHistograms(TFile* file) 
{
   if (fInfoList) { 
      TIter next(fInfoList);
      MethodInfo *info(0);
...
void StatDialogMVAEffs_upperlimit::DrawHistograms() 
{
   // counter variables
   Int_t countCanvas = 0;
   ...
   while ( (info = (MethodInfo*)next()) ) {
      ...
      legend2->AddEntry(info->sSig,"S / #sqrt{B}","L");
      legend2->Draw("same");
      ...
      tl.DrawLatex( 0.15, 0.19, "events the maximum S / #sqrt{B} is");
      ...
      if (Save_Images) {
         TMVAGlob::imgconv( c, Form("plots/MVAEffs_upperlimit_%s", info->methodTitle.Data()) ); 
      }
      countCanvas++;
   }
}

void StatDialogMVAEffs_upperlimit::PrintResults( const MethodInfo* info )
{
   Int_t maxbin = info->sSig->GetMaximumBin();
   if (info->line1 !=0 )
...
//void MVAEffs_upperlimit( TString fin = "TMVA.root", Bool_t useTMVAStyle = kTRUE, TString formula="S/sqrt(S+B)" )
void mvaeffs_upperlimit( TString fin = "TMVA.root", Bool_t useTMVAStyle = kTRUE, TString formula="S/sqrt(B)" )
//void mvaeffs_upperlimit( TString fin = "TMVA.root", Bool_t useTMVAStyle = kTRUE, TString formula="S/(sqrt(B) + 2.0)" )
{
   TMVAGlob::Initialize( useTMVAStyle );

   StatDialogMVAEffs_upperlimit* gGui = new StatDialogMVAEffs_upperlimit(gClient->GetRoot(), 1, 1);
