...
class LASSevent: 
  ...
  def __init__(self, tree=None, composite_particles=[]): # TTree object
    '''Constructor for LASSevent() object'''
  ...
  ##############################################################################
  def MyTree(self): # 
    '''Return the TTree object with which the LASSevent has been initialized.'''
    return self._tree
  ...
  def GetParticle(self, particle="X", n=0): #
    '''Return a LASSparticle of the \'n\'th occurance of \'X\'.'''
    ...
    ret = LASSparticle(particle, lund, p4s, vertex, isComposite, vals_float, vals_int, pid_bits)
    return ret


  ##############################################################################
  def IsParticleInTree(self, particle="X"):
    '''Check to see if \'particle\' is in the TTree object passed in to this LASSevent.'''
    i = -1
    try:
      i = self.particle_names.index( particle )
    except ValueError:
  ...
  def IsStringInList(self, mylist=[], mystring="X"):
    '''Check to see if \'mystring\' is in any of the lists in this LASSevent.
