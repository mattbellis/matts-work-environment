...
for f in range(0,1):
  ...
  if os.path.isfile(infilename ):
    ...
    for i in range(0,6):
      for j in range(0,6):
        for k in range(0,6):
          hname = "hmass0_0_" + str(36*i + 6*j + k + 3)
          #print hname
          ...
          histos[i][j][k].SetFillColor(33)
          if i==0 and j==0 and k==0: 
            histos[i][j][k].SetFillColor(41)
          #print "here " + str(hix)
          ...
          if j==0 and k==1:
            ...
            hleg[legcount].AddEntry(histos[0][0][0],"m_{ES}/#Delta E range","f")
            #hleg[legcount].AddEntry(hbase[2],"P(#Chi^{2})>0.001","f")
            hleg[legcount].AddEntry(histos[i][j][k],"Selectors cut","f")
            hleg[legcount].Draw()
            legcount += 1
...
lamrangelo = lammass - 0.010
lamrangehi = lammass + 0.010
lox = 2.24
hix = 2.32
lobin = 1
hibin = 1
...
for i in range(0,6):
  ...
  for j in range(0,6):
    ...
    for k in range(0,6):
      ...
      # Fit
      histos[i][j][k].Fit("testfit", "ERFQN","")
      # Redraw
...
for i in range(0,216):
  if (abs(sigevents[i]/maxsig)<1.0 and abs(1.0 - (bakevents[i]/maxbak))<1.0):
    ypts.append(sigevents[i]/maxsig)
    xpts.append(1.0 - (bakevents[i]/maxbak))
    print str(i) + " " + str(xpts[i]) + " " + str(ypts[i]) + "\n"
"""
...
for g in range(0,18):
  for i in range(0,6):
    for j in range(0,6):
      for k in range(0,6):
        ...
        sigpct = sigevents[i][j][k]/maxsig
        bakpct = 1.0 - (bakevents[i][j][k]/maxbak)
  ...
  grroc[g].SetMarkerColor(g%6 + 1)
  grroc[g].SetMarkerStyle(20)
  grroc[g].SetMarkerSize(2.0)
  ...
  # hsig
  ###########################
  hsig[g].SetMarkerColor(g%6 + 1)
  hsig[g].SetMarkerStyle(23)
  hsig[g].SetMarkerSize(2.0)
  
  hsig[g].SetTitle("")
  hsig[g].SetMaximum(1.2) 
  ...
  # hbak
  ###########################
  hbak[g].SetMarkerColor(g%6 + 1)
  hbak[g].SetMarkerStyle(22)
  hbak[g].SetMarkerSize(2.0)
...
for j in range(0,6):
  name = "Plots/sigbkg_selectors_allplots_" + tag + "_" + str(j) + ".eps" 
  can[j].SaveAs(name)
for j in range(0,1):
  name = "Plots/sigbkg_selectors_roc_" + tag + "_" + str(j) + ".eps" 
  gcan.SaveAs(name)
for j in range(0,3):
  name = "Plots/sigbkg_selectors_effs_" + tag + "_" + str(j) + ".eps" 
